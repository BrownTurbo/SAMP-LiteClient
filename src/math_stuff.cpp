...
void vect2_normalize ( const float in[2], float out[2] )
{
	...
	m = std::sqrt( in[0] * in[0] + in[1] * in[1] );
...
{
	//if ( !std::isfinite(in[0]) || !std::isfinite(in[1]) )
	//	return 1;
...
{
	std::memcpy( out, in, sizeof(uint8_t) * 2 );
}
...
void vect3_normalize ( const float in[3], float out[3] )
{
	...
	m = std::sqrt( in[0] * in[0] + in[1] * in[1] + in[2] * in[2] );
...
{
	//if ( !std::isfinite(in[0]) || !std::isfinite(in[1]) || !std::isfinite(in[2]) || !std::isfinite(in[3]) )
	//	return 1;
...
{
	std::memcpy( out, in, sizeof(uint8_t) * 4 );
}
...
void matrix_vect3_rotate ( const float in[16], const float vect[3], float t, float out[16] )
{
	...
	const float sin_t = std::sin( t ), cos_t = std::cos( t );
	float		res[16];
