...
bool RakPeer::HandleRPCPacket(const char *data, int length, PlayerID playerId)
{
	...
	/*if (uniqueIdentifier != (int *)RPC_UpdateScoresPingsIPs)
	{
		...
		AppendLogF("[RPC] uniqueIdentifier: %d", uniqueIdentifier);
...
#endif
{
	...
	else
	{
		...
		if (((unsigned char)data[0] == ID_PING_OPEN_CONNECTIONS || (unsigned char)(data)[0] == ID_PING) && length == sizeof(unsigned char) + sizeof(RakNetTime))
		{
...
bool RakPeer::RunUpdateCycle(void)
{
	...
	while (rcs)
	{
		...
		if (rcs->nextRequestTime < timeMS)
		{
			...
			c[0] = ID_OPEN_CONNECTION_REQUEST;
			//*(uint16_t*)&c[1] = (uint16_t)rcs->playerId.port ^ NETCODE_OPENCONNLULZ; // dropped in 0.3.7-RC5-1
			...
			while (bitSize > 0)
			{
				...
				else
				{
					...
					/*if ((unsigned char)(data)[0] != 16)
						AppendLogF("%.2X %d", (unsigned char)(data)[0], byteSize);//*/
